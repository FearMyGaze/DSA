...
public class FileController {
    ...
    public static void fileDelete (Context context, int userID, int fileID, IVolleyMessage iVolleyMessage){
        String[] url = context.getResources().getStringArray(R.array.url);
        String jsonError = context.getResources().getString(R.string.jsonErrorDuring);
        String errorOnFileDelete = context.getResources().getString(R.string.errorOnFileDelete);
        String volleyError = context.getResources().getString(R.string.volleyError);

        String userid = Integer.toString(userID);
        String id = Integer.toString(fileID);

        StringRequest request = new StringRequest(Request.Method.POST, url[8],
                response -> {
                    try {
                        JSONObject jsonResponse = new JSONObject(response);
                        if (jsonResponse.getString("success").equals("1")) {
                            iVolleyMessage.onSuccess(context.getResources().getString(R.string.successFileDeletion));
                        } else {
                            iVolleyMessage.onWaring(errorOnFileDelete);
                        }
                    } catch (JSONException e) {
                        iVolleyMessage.onError(jsonError + " " + e.getMessage());
                    }
